...
// Simple Solidity demo for upload images and add timestamp to blockchain
contract TimestampDemo {
  ...
  function TimestampDemo() payable {  // this is the CONSTRUCTOR (same name as contract) it gets called ONCE only when contract is first deployed
    AdminUser = msg.sender;  // just set the admin, so they can remove bad users or images if needed, but nobody else can
  }
